...
function comparatorBounds(raw: string): Bound[] | null {
  ...
  if (c.startsWith("^") || c.startsWith("~")) {
    ...
    const explicitParts =
      (body.split("+")[0] ?? "").split("-")[0]?.split(".").length ?? 1;
...
const SKIP_PROTOCOLS =
  /^(workspace:|link:|file:|npm:|git|github:|https?:|portal:)/;
...
function analyze(
  ...
  installed: Installed[],
  registryData:
    { distTags: Record<string, string>; versions: string[] } | Error,
): PackageResult {
  ...
  const published = registryData.versions
    .map(parseSemver)
  ...
  if (preId) {
    ...
    const sameLine = published.filter(
      (v) => v.major === current.major && prereleaseId(v) === preId,
    );
    target = sameLine.reduce<SemVer | null>(
      (a, b) => (!a || compareSemver(b, a) > 0 ? b : a),
      null,
    );
    ...
    target =
      newestStable && compareSemver(newestStable, current) > 0
        ? newestStable
        : current;
  }
...
function printHuman(
  ...
  const nameWidth = Math.min(
    34,
    Math.max(...results.map((r) => r.name.length), 12),
  );
  ...
  for (const group of groups) {
    ...
    for (const r of groupResults) {
      ...
      if (r.error) {
        console.log(
          `  ${yellow("!")} ${name}  ${dim(`lookup failed: ${r.error}`)}`,
        );
        continue;
      }
      if (r.declarations.length === 0 && r.installed.length === 0) {
        console.log(
          `  ${dim("·")} ${name}  ${dim("not declared in this workspace")}`,
        );
        continue;
      }
      ...
      if (r.updateKind === "none") {
        console.log(
          `  ${green("✓")} ${name}  ${current}  ${dim("up to date")}`,
        );
      } else {
        console.log(
      ...
      if (verbose) {
        for (const inst of r.installed) {
          console.log(
            `      ${dim(`installed ${inst.version}  ${inst.path}`)}`,
          );
        }
      }
      ...
      bucket.verdict === "allows"
        ...
        : bucket.verdict === "blocks"
          ...
          : bucket.verdict === "unmodelled"
            ...
            : "";
    const count =
      bucket.sources.length > 1 ? dim(` ×${bucket.sources.length}`) : "";
    lines.push(
...
function printSummary(results: PackageResult[]): void {
  ...
  if (updates.length > 0) {
    ...
    const needsBump = updates.filter(
      (r) =>
        r.rangesAllowingTarget.length === 0 &&
        r.rangesBlockingTarget.length > 0,
    );
    console.log();
...
async function main() {
  ...
  const manifest = readJSON<KeyLibraries>(
    join(ROOT, "tools/key-libraries.json"),
  );
  if (!manifest?.groups) {
  ...
  if (opts.groups.length > 0) {
    ...
    if (groups.length === 0) {
      console.error(
        `No matching groups. Available: ${manifest.groups.map((g) => g.id).join(", ")}`,
      );
      process.exit(2);
    }
  }
  ...
  for (const g of groups)
    for (const p of g.packages) if (!groupOf.has(p)) groupOf.set(p, g.id);
  ...
  if (!opts.json) {
    console.log(
      `Checking ${names.length} key libraries against ${opts.registry} …`,
    );
  }
  ...
  const results: PackageResult[] = names.map((name, index) => {
    ...
    const dirs =
      decls.length > 0 ? [...new Set(decls.map((d) => d.dir))] : [ROOT];
    const installed = resolveInstalled(name, dirs);
    const registryData =
      fetched[index] ?? new Error("no registry response recorded");
    return analyze(
      name,
      groupOf.get(name) ?? "unknown",
      decls,
      installed,
      registryData,
    );
