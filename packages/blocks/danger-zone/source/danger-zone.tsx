...
export function DangerZone(props: DangerZoneProps): JSX.Element {
  const [state, setState] = createSignal<DangerZoneState>(
  ...
  async function handleAction(action: () => Promise<void> | undefined, label: string) {
    if (confirmAction() !== label) { setConfirmAction(label); return }
  ...
  return (
    <div class={["solidiom-block-danger-zone", props.class].filter(Boolean).join(" ")} data-state={state()}>
      ...
      <Show when={state() === "error" && currentError()}>
        <div class="solidiom-block-danger-zone__error" role="alert"><p>{currentError()}</p></div>
      </Show>
      <Show when={state() !== "restricted"}>
        <div class="solidiom-block-danger-zone__actions">
          <div class="solidiom-block-danger-zone__action">
            <div><h3>Export Data</h3><p>Download all your data as a ZIP archive.</p></div>
            <button type="button" onClick={() => handleAction(() => props.onExportData?.(), "export")} disabled={state() === "loading"}>
              {confirmAction() === "export" ? "Confirm Export" : "Export Data"}
            </button>
          </div>
          <div class="solidiom-block-danger-zone__action">
            <div><h3>Deactivate Account</h3><p>Temporarily disable your account. You can reactivate later.</p></div>
            <button type="button" onClick={() => handleAction(() => props.onDeactivate?.(), "deactivate")} disabled={state() === "loading"}>
          ...
          <div class="solidiom-block-danger-zone__action solidiom-block-danger-zone__action--destructive">
            <div><h3>Delete Account</h3><p>Permanently delete your account and all data. This cannot be undone.</p></div>
            <button type="button" onClick={() => handleAction(() => props.onDeleteAccount?.(), "delete")} disabled={state() === "loading"}>
