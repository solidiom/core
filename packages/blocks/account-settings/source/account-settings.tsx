...
export function AccountSettings(props: AccountSettingsProps): JSX.Element {
  ...
  return (
    <div class={["solidiom-block-account-settings", props.class].filter(Boolean).join(" ")} data-state={state()}>
      ...
      <Show when={state() === "error" && currentError()}>
        <div class="solidiom-block-account-settings__error" role="alert"><p>{currentError()}</p></div>
      </Show>
      <Show when={state() !== "restricted"}>
        <form onSubmit={handleSave} class="solidiom-block-account-settings__form">
          <div class="solidiom-block-account-settings__field">
            ...
            <input id="settings-name" type="text" value={name()} onInput={(e) => setName(e.currentTarget.value)} disabled={state() === "loading"} />
          </div>
          <div class="solidiom-block-account-settings__field">
            <label for="settings-email">Email</label>
            <input id="settings-email" type="email" value={email()} onInput={(e) => setEmail(e.currentTarget.value)} disabled={state() === "loading"} />
          </div>
          <button type="submit" class="solidiom-block-account-settings__submit" disabled={state() === "loading"}>
            <Show when={state() === "loading"} fallback="Save Changes">Saving...</Show>
          </button>
        </form>
      </Show>
    </div>
