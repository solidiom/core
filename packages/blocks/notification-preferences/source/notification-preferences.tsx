...
export function NotificationPreferences(props: NotificationPreferencesProps): JSX.Element {
  ...
  return (
    <div
      class={["solidiom-block-notification-preferences", props.class].filter(Boolean).join(" ")}
      data-state={state()}
    >
      ...
      <Show when={state() === "error" && currentError()}>
        <div class="solidiom-block-notification-preferences__error" role="alert">
          <p>{currentError()}</p>
        </div>
      </Show>
      <Show when={state() !== "restricted"}>
        <Show when={categories().length === 0}>
          <div class="solidiom-block-notification-preferences__empty">
            <p>No notification categories configured.</p>
          </div>
        </Show>
                    ...
                    {(ch) => (
                      <label class="solidiom-block-notification-preferences__toggle">
                        <input
                          type="checkbox"
                          checked={ch.enabled}
                          onChange={(e) => handleToggle(cat.id, ch.id, e.currentTarget.checked)}
        ...
        <Show when={categories().length > 0}>
          <button
            type="button"
            class="solidiom-block-notification-preferences__save"
            onClick={handleSave}
            disabled={state() === "loading"}
          >
            <Show when={state() === "loading"} fallback="Save Preferences">
              Saving...
            </Show>
          </button>
        </Show>
