...
function createMinimalPositioning(options = {}) {
  const compute = (input) => {
    ...
    if (options.flip && options.viewport) {
      ...
      const shouldFlip = side === "top" && y < 0 || side === "bottom" && y + floating.height > vh || side === "left" && x < 0 || side === "right" && x + floating.width > vw;
      if (shouldFlip) {
...
function computePlacement(ref, floating, placement, offset) {
  const side = placement.split("-")[0];
...
function flipPlacement(placement) {
  ...
  return alignment ? `${newSide}-${alignment}` : newSide;
}
export {
  createMinimalPositioning
};
