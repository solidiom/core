...
export function createTimeMath(): TimeMath {
  ...
  const parse = (text: string): TimeValue | null => {
    ...
    const match12 = trimmed.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM|am|pm)$/)
    if (match12) {
    ...
    const match24 = trimmed.match(/^(\d{1,2}):(\d{2})(?::(\d{2})(?:\.(\d{1,3}))?)?$/)
    if (match24) {
      const hour = parseInt(match24[1]!, 10)
      const minute = parseInt(match24[2]!, 10)
      const second = match24[3] !== undefined ? parseInt(match24[3]!, 10) : 0
      const millisecond = match24[4] !== undefined ? parseInt(match24[4]!.padEnd(3, "0"), 10) : 0
  ...
  const toSeconds = (time: TimeValue): number => time.hour * 3600 + time.minute * 60 + time.second
  ...
  const getDayPeriod = (time: TimeValue): DayPeriod => (time.hour < 12 ? "AM" : "PM")
