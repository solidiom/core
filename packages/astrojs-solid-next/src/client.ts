import { Loading, createStore } from 'solid-js';
...
export default (element: HTMLElement) =>
	(Component: any, props: any, slotted: any, { client }: { client: string }) => {
		...
		if (alreadyInitializedElements.has(element)) {
			// update the mounted component — apply new props via draft mutation
			const newProps = { ...props, ...slots, children };
			alreadyInitializedElements.get(element)!((state: any) => {
