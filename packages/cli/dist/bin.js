...
function countImports(packageName, cwd) {
  const srcDir = join10(cwd, "src");
  if (!existsSync10(srcDir)) return 0;
  try {
    const result = execSync(
      `grep -r "${packageName}" "${srcDir}" --include="*.ts" --include="*.tsx" -l 2>/dev/null | wc -l`,
      { encoding: "utf8", cwd, timeout: 1e4 }
    );
...
function isInstalled(packageName, cwd) {
  const fullPath = join10(cwd, "node_modules", ...packageName.split("/"));
  return existsSync10(fullPath);
}
function runLegacyStatus(cwd) {
  const facades = KNOWN_FACADES.map((spec) => ({
    ...spec,
...
function runLegacyAdd(facadeName, cwd) {
  ...
  try {
    execSync(`pnpm add ${spec.name}`, { cwd, encoding: "utf8", stdio: "pipe", timeout: 6e4 });
    return {
      ok: true,
...
function runLegacyRemove(facadeName, cwd) {
  const spec = KNOWN_FACADES.find((f) => f.name === facadeName || f.name.endsWith(facadeName));
  if (!spec) {
    const available = KNOWN_FACADES.map((f) => f.name).join(", ");
    return { ok: false, message: `Unknown facade "${facadeName}". Available: ${available}` };
  ...
  try {
    execSync(`pnpm remove ${spec.name}`, { cwd, encoding: "utf8", stdio: "pipe", timeout: 6e4 });
...
var LegacyCommand = class extends Command11 {
  static paths = [["legacy"]];
  ...
  json = Option11.Boolean("--json", false, { description: "Output as JSON" });
  async execute() {
    return printStatus(this, process.cwd(), this.json);
  }
};
var LegacyStatusCommand = class extends Command11 {
  static paths = [["legacy", "status"]];
  static usage = Command11.Usage({
...
var LegacyAddCommand = class extends Command11 {
  static paths = [["legacy", "add"]];
  static usage = Command11.Usage({
  ...
  facade = Option11.String({ required: true });
  async execute() {
    const result = runLegacyAdd(this.facade, process.cwd());
    if (result.ok) {
      this.context.stdout.write(pc11.green(`\u2713 ${result.message}
`));
      return 0;
    }
    this.context.stderr.write(pc11.red(`\u2717 ${result.message}
`));
    return 1;
  }
};
var LegacyRemoveCommand = class extends Command11 {
  static paths = [["legacy", "remove"]];
  static usage = Command11.Usage({
...
function printStatus(cmd, cwd, json) {
  const result = runLegacyStatus(cwd);
  if (json) {
    cmd.context.stdout.write(JSON.stringify(result, null, 2) + "\n");
    return 0;
  }
  cmd.context.stdout.write(pc11.bold("\nLegacy Facades:\n\n"));
  for (const facade of result.facades) {
    const status = facade.installed ? facade.importCount > 0 ? pc11.yellow(`installed, ${facade.importCount} file(s) importing`) : pc11.green("installed, ready to remove") : pc11.dim("not installed");
    cmd.context.stdout.write(`  ${pc11.bold(facade.name)} [${status}]
`);
...
`
    ...
    if (facade.installed && facade.importCount === 0) {
      cmd.context.stdout.write(
        pc11.green(`    \u2192 Safe to remove: solidiom legacy remove ${facade.name}
`)
      );
    }
    cmd.context.stdout.write("\n");
  ...
  if (result.readyToRemove.length > 0) {
    cmd.context.stdout.write(
      pc11.green(`${result.readyToRemove.length} facade(s) ready to remove.
`)
    );
  }
  if (result.totalImports > 0) {
    cmd.context.stdout.write(
      pc11.dim(`${result.totalImports} total imports remaining across all facades.
`)
    );
  }
  return 0;
