...
describe("package consumer proofs", () => {
  ...
  beforeAll(() => {
    ...
    const runtimeOut = exec(`pnpm --filter @solidiom/runtime pack --pack-destination ${PACK_DIR}`)
    runtimeTarball = runtimeOut.trim().split("\n").pop()!.trim()

    const dialogOut = exec(`pnpm --filter @solidiom/dialog pack --pack-destination ${PACK_DIR}`)
