...
describe("declarationToUtilities — box spacing shorthand", () => {
  it("maps a uniform padding to a single p- utility", () => {
    // The class-string path (renderVariantsModule) wraps its output in
    // tailwind-merge's twMerge(), which resolves Tailwind's own utility-group
    // ordering (docs/contracts/recipe-contract.md §6), so this function is free to
    // emit whichever form reads most naturally — see boxUtility's doc comment.
  ...
  it("maps zero padding to a single p-0 utility", () => {
