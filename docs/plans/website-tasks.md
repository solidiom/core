...
`RECIPE-001` is complete; `docs/contracts/recipe-contract.md` is the normative reference and `docs/contracts/recipe-authoring-guide.md` is the authoring workflow. It delivered the contract, vocabulary, token identities, exception model, and validator only. Emitters and parity assertions remain deferred to `RECIPE-002..006`. `RECIPE-002` is in progress: its definition foundation now covers all 13 shipped CSS recipes, has a CI-enforced wrapper/slot coverage guard, models Alert from its emitted `data-state`, and records adapter-owned Popover/Tooltip geometry. CSS emission and migration remain outstanding. Hand-authored recipes must already follow the contract's rules, which is what makes `RECIPE-002/003/004` migrations rather than rewrites.
