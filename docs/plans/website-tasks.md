...
**Status:** in execution — M0–M2 complete; M3 complete on content, evidence not yet enforced by an observed CI run; M4 in progress
...
Current position is in §9. `M4` is in progress: 14 primitives have bilingual catalog pages, 0 meet the full Primitive DoD in §8.1. `M5` has begun incidentally (`MKT-005`, `BUILDER-008`).

> **Read first.** `.github/workflows/ci.yml` did not parse for the whole span in which G3 was declared complete: `phase3-gate`'s final step was mis-indented, so GitHub rejected the workflow outright and **no job in it ever ran** — not the gates added to close earlier drift, not `format`, not `typecheck`, not `build`. The syntax is now fixed and the file loads 22 jobs, but no green run has been observed yet, and `format` is known to fail on 132 files (`CI-006`). Until a full run is observed, treat the evidence below as locally verified rather than enforced; each figure records the command that produced it. See `CI-005` (§11.1).
...
| Limitation                                                                                                                                                                                                                                                                     | Owner        |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ |
| No CI job had ever run: `.github/workflows/ci.yml` was invalid YAML, so every "blocking" claim in this document went unenforced. The syntax is fixed and the file now loads 22 jobs; a green run has not been observed, and `format` will fail until `CI-006` lands.           | `CI-005`     |
...
| Typeset/prose recipes ship in the CSS and Tailwind profiles only, and are invisible to `audit:recipe-parity` because they are not declared in the canonical contract.                                                                                                          | `RECIPE-007` |
| The committed `registry/*.json` predates every M4 catalog commit and describes eight shipped primitives as documentation stubs with no accessibility evidence. CI cannot self-heal it: `registry:build` runs in a job that finishes before the evidence-generating job starts. | `A11Y-009`   |
| 132 files fail `prettier --check`, violating global DoD rule 10.                                                                                                                                                                                                               | `CI-006`     |
...
Do not promote these rows to `[x]` until `A11Y-009` and `I18N-005` close and `PRIM-000` (§11.1) exists to re-check the claim. Note also that `gate:vertical-slice` counts complete-status primitive rows in this file and fails when it finds any, so `VS-005` must land before the first of these rows can close.
...
- [ ] `PRIM-001..052` complete: exactly 52/52. Currently 0 complete, 14 in progress (11 from M4 plus the three vertical-slice primitives).
...
| [ ]    | TEST-005   | M    | Visual QA        | **Corrected:** the 36/36 local failure is a platform difference, not stale content. Set up the containerised visual harness (`mcr.microsoft.com/playwright:v1.61.1-noble`) so baselines are verifiable on any OS; observe one CI run; approve or regenerate. Schedule after the last primitive (38 new pages change `/primitives/`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [ ]    | CI-007     | XS   | CI               | Promote `beta-acceptance-e2e` from `continue-on-error: true` to blocking now that it passes 111/111, and correct its inline comment, which still describes the pre-`A11Y-008` `color-contrast` failures as current. Sequence after `CI-005` so the promotion can be observed taking effect.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| [ ]    | RECIPE-007 | M    | Design systems   | Bring typeset/prose into the canonical contract and the UnoCSS profile. `packages/recipes-css/src/styles/{typeset,prose}.css` and `packages/recipes-tailwind/src/recipes/typeset.tsx` have no UnoCSS counterpart, and because the scopes are absent from `tools/recipe-contract-definitions.ts`, `audit:recipe-parity` reports parity regardless. See §7.1.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`Primitives / Landed = 14` is the 11 M4 primitives plus Combobox, Data Table, and Dialog. `DoD = 0` because no primitive satisfies §8.1's registry-currency and human-reviewed-Spanish requirements (`A11Y-009`, `I18N-005`). All 52 already have a generated API artifact, a registry manifest, an `artifacts/a11y-evidence.json` entry, and four generated routes, so route and API counts are not a proxy for catalog completion — the committed registry currently describes eight of the eleven M4 primitives as documentation stubs.
