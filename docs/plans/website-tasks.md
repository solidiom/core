...
| [~]    | CLI-003    | M    | REG-006, CLI-002         | CLI/security      | Verified manifests/hashes gate source installs; lock/provenance records. In-repo verification complete (`verify-source.ts`, `lock.ts`, `PolicySchema`, `--allow-unverified`). CI signing and `provenance: "unverified"` assertion blocked on OPS-002.                                                                       |
...
| M3 Public beta platform   | In progress | G3   | RECIPE-001 complete: canonical recipe contract, semantic vocabulary (14 attributes, 35 known scopes — 33 stateful plus stateless Badge/Toast — 8 flags, 9 recorded exceptions), 48 token identities, exception model, validator, and Button/Switch/Dialog reference definitions, published as `docs/contracts/recipe-contract.md` and enforced by `gate:phase1` §9 on every pull request. RECIPE-002 is in progress: all 13 CSS recipe scopes have validated canonical definitions, and CI guards wrapper/slot coverage; CSS emission/migration remains. THEME-001..005 are complete: versioned semantic theme JSON schema (`tools/theme-contract-schema.ts`), validator with a WCAG-floor contrast check (`tools/theme-contract-validate.ts`, `tools/theme-contract-contrast.ts`), the first migration mechanism in the repo (`tools/theme-contract-migrate.ts`), a `solidiom-default` reference theme cross-checked against BRAND-002's `tokens.css` for drift, CSS/Tailwind/UnoCSS generation (`tools/theme-emit-{css,tailwind,unocss}.ts` into the new `packages/themes` and `packages/unocss-preset`'s `presetSolidiom({ theme })`), and a cross-output parity/contrast-matrix/round-trip audit (`tools/audit-theme-parity.ts`, `pnpm run audit:theme-parity`), published as `docs/contracts/theme-contract.md`. THEME-005's contrast matrix initially surfaced one genuine finding — `solidiom-default`'s light-mode `primary-foreground`/`primary` pair at 4.36:1, just under the 4.5:1 WCAG AA minimum — resolved by darkening the light-mode primary (and its `border-active`/`focus-ring` aliases) from `#6D66F1` to `#6961F1` in `apps/site/src/assets/tokens.css` and `tools/theme-contract-definitions.ts` (same hue/saturation, ~1 point less HSL lightness; now 4.58:1); the literal brand mark assets intentionally keep the original `#6D66F1`. `audit:theme-parity` now passes with zero findings and runs as a fully blocking CI step. CLI-001 is complete: `packages/cli/src/source/` renamed to `src/source-install/` to stop colliding with the `src/`→`source/` emission root; `tools/audit-package-source-parity.ts` now generalizes RECIPE-006's parity audit to cover the CLI's and `themes`' tooling-tier source trees, `tools/emit-package-source.ts` adds an explicit regeneration/check script, and `gate:phase1` §9b asserts the collision is structurally resolved. Remaining M3 work: RECIPE-002b, RECIPE-003..006, CLI-002..010, BUILDER-001..006, BETA-001..003. |
