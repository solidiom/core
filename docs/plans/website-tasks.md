...
**Status:** in execution — M0–M2 complete (G0–G2 passed); M3 in progress
...
Current position is in §11. `M3`'s remaining work is `BETA-002`, `A11Y-007`, and `A11Y-008`; `M4` and `M5` have not started.
...
- [x] No bulk catalog work has bypassed the vertical-slice gate — asserted by `gate:vertical-slice` §11, blocking in CI.
...
`RECIPE-001` is complete; `docs/contracts/recipe-contract.md` is the normative reference and `docs/contracts/recipe-authoring-guide.md` is the authoring workflow. `RECIPE-002/003/004` are complete: the CSS, Tailwind, and UnoCSS emitters generate every shipped recipe (13 primitives × 3 profiles) from `tools/recipe-contract-definitions.ts`, with `pnpm run recipe:emit:{css,tailwind,unocss}:check` enforced in `gate:phase1`. `recipes-unocss` now ships a full recipe catalog matching the other two profiles, closing the gap it previously declared. `RECIPE-005/006` are complete: `tools/audit-recipe-parity.ts` asserts cross-profile coverage/state/exception parity and `tests/recipe-parity/` asserts computed-style parity for a rendered fixture (both enforced in `gate:phase1`/CI); `tools/audit-package-source-parity.ts` and the `tests/package-source-parity` suite assert `src`/`source` byte parity and export-map completeness for all three recipe packages.
...
| [x]    | CLI-009    | S    | CLI-002..008             | Documentation     | Bilingual CLI documentation: 8 EN + 8 ES guides with real `translationSourceHash`. `docs/guides/offline-install.md` folded into `cli-recovery.md`. Commands/flags/packages untranslated.                                                                                                                                                                           |
...
| [ ]    | THEME-006   | L    | THEME-005, RECIPE-003        | Design systems       | Promote the type scale into the canonical token model as paired `font-size`/`line-height` identities with themeable values, and emit both from every profile. Non-blocking for G3; see §7.3. |
...
| [ ]    | TEST-005 | M    | TEST-003                                             | Visual QA           | Regenerate and human-approve the 36 visual baselines on Linux CI; they currently mismatch 36/36. Until then `site-visual` carries no signal.                                                                                                                                   |
| [ ]    | TEST-006 | M    | TEST-002                                             | QA                  | Stabilize the intermittently-failing E2E tests: `beta-acceptance` search keyboard navigation, and `shell-a11y`/`shell-smoke` mobile drawer and theme toggle. Each passes in isolation and fails under parallel load, so suite results currently need re-running to interpret.  |

#### BETA-002 status

`BETA-002` is **in progress, not complete.** Its static-build half passes — `beta:acceptance:report` reports 60/60 across route existence, locale parity, and search index. Its cross-browser half does not: `beta:acceptance:e2e` fails 12 of 111 checks deterministically, all on `color-contrast`, covering `/`, `/primitives/`, `/themes/builder/`, and `/privacy/` in Chromium, Firefox, and WebKit.

A thirteenth check, `Search › keyboard_accessible`, fails intermittently — 8 of 18 runs when repeated in isolation, in varying browsers. It is flaky rather than broken, and is tracked with the suite's other unstable tests under `TEST-006`.

Counts above were measured locally in parallel. The `beta-acceptance-e2e` CI job confirms the same two failure classes on Linux — `no_critical_violations` from `color-contrast`, and the flaky `keyboard_accessible` — but its raw totals differ, because that config runs `workers: 1` with `retries: 2`, so an intermittent failure can pass on retry.
...
Two defects found alongside it were fixed rather than tracked, since neither needed a design decision:

- `BetaBanner.tsx` carried `role="banner"` on a `div` while the shell `<header>` already owns that landmark, producing two banner landmarks and failing `landmarks_present`. ARIA permits one per document; the role is removed.
- `apps/site/tests/e2e/shell-a11y.spec.ts` had been changed from `toHaveCount(1)` to `toHaveCount(2)` banners (commit `1930bfc`, "resolve remaining CI failures") to accommodate that defect rather than fix it, and its title lost the word "one". Both are restored to the SITE-011 requirement.

### G3 exit checklist

- [x] Canonical recipe contract and all three emitters ship every recipe scope from one definition (`RECIPE-001`..`006`).
- [x] Theme contract, generation, and cross-output parity/contrast/round-trip audits pass as blocking CI steps (`THEME-001`..`005`).
- [x] CLI covers plan/inspect/add/create with verified manifests, conflict/diff/rollback, four package managers, and offline fixtures (`CLI-001`..`010`).
- [x] Theme-builder shell, editor, preview grid, import/export, and share state ship behind route-local boundaries (`BUILDER-001`..`006`).
...
- [ ] Cross-browser acceptance matrix passes (`beta:acceptance:e2e`) — blocked by `A11Y-008`.
...
Recorded rather than left implicit, per global Definition of Done rule 1. None of these blocks G3; each has an owner.

| Limitation                                                                                                                                                                        | Owner       |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| Typography is not tokenized. `font-size` is a literal in recipe definitions, and `line-height` is declared ad hoc, so the type scale is neither themeable nor contrast-validated. | `THEME-006` |
| Visual-regression evidence is unverified. All 36 baselines mismatch, so `site-visual` currently proves only that the suite runs.                                                  | `TEST-005`  |
| `site-lighthouse` remains advisory; performance budgets are defined but not enforced for the site.                                                                                | `QA-004`    |
| Several E2E tests fail intermittently under parallel load, so a single red run is not by itself evidence of a defect.                                                             | `TEST-006`  |
...
| M3 Public beta platform   | In progress | G3   | `RECIPE-001`..`006`, `THEME-001`..`005`, `CLI-001`..`010`, `BUILDER-001`..`006`, `BETA-001` and `BETA-003` are complete and evidenced by blocking CI steps: `gate:phase1` §9/§9b, `recipe:emit:{css,tailwind,unocss}:check`, `audit:recipe-parity`, `test:recipe-parity`, `theme:emit:*:check`, `audit:theme-parity`, `audit:package-source-parity`, `assert:no-unverified`, and the four-package-manager offline `cli-smoke-create` matrix. **M3 is not complete.** `BETA-002` is in progress: its static-build acceptance report passes 60/60, but its cross-browser matrix fails 12 of 111 checks on `color-contrast`, rooted in a single site token (`--sol-secondary` at 3.678:1 against white, below the 4.5:1 AA floor). Neither acceptance script had ever been wired into a workflow, which is why this row and its task rows disagreed for several commits; both are now `ci.yml` jobs. `gate:phase3` is additionally advisory pending three unrecorded manual accessibility artifacts. Remaining M3 work: `BETA-002`, `A11Y-007`, `A11Y-008`. Non-blocking accepted limitations (`THEME-006`, `TEST-005`, `TEST-006`, `QA-004`) are recorded in §7.3. A previous version of this row described `RECIPE-002` as in progress and listed `RECIPE-003`..`006`, `CLI-002`..`010`, `BUILDER-001`..`006` and `BETA-001`..`003` as outstanding; all of that was stale. It also reported a `primitive-completion-gate` failure against `recipes-unocss`, which was a bug in the gate — it read `src/index.ts` instead of `src/meta.ts` for that profile alone — now fixed, with all 52 primitives passing. |
...
Counters track **catalog-item** completion against the M4 queues, so every row except `Locales` is legitimately `0` until `PRIM-*`/`COMP-*`/`BLOCK-*`/`TPL-*`/`PRESET-*` work begins after `VS-004`. They are not a measure of M0–M3 progress.

`Locales` counts locales _implemented and enforced_, not per-item translation completeness: English and Spanish both ship with explicit locale context, route-parity validation, canonical/`hreflang` metadata, and translation-freshness checks (`I18N-001`..`004`), and `beta:acceptance:report` asserts 45/45 locale-parity checks. Per-item Spanish review remains part of each catalog item's Definition of Done in §8.5, so this row reaching 2 does not imply the catalog is bilingual.
