...
> **Infrastructure resolved.** `.github/workflows/ci.yml` parses and loads 22 jobs (CI-005). `format:check` passes (CI-006). `beta-acceptance-e2e` is blocking (CI-007). The vertical-slice gate delegates to PRIM-000 (VS-005). The BUILD-001 staleness guard is active and satisfiable — generated artifacts no longer carry wall-clock or HEAD-derived stamps. `site-visual` is containerised and blocking (TEST-005). The tools test suite runs in CI (`test:tools`), where 17 of its 35 files previously ran nowhere. Registry and evidence files are committed and current (A11Y-009). Typeset/prose are in the canonical contract across all three profiles (RECIPE-007). BLOCK-000A resolved via `proposedComponents`. Visual baselines pass 36/36 in the pinned container and in CI (TEST-005). A full CI run has now been observed: run `30871761546` was green across all 31 jobs, including the eleven that had never executed.
...
pnpm run gate:phase1              # expect 249/249
