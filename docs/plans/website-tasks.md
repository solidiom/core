...
| M3 Public beta platform   | In progress | G3   | RECIPE-001 complete: canonical recipe contract, semantic vocabulary (14 attributes, 35 known scopes — 33 stateful plus stateless Badge/Toast — 8 flags, 9 recorded exceptions), 48 token identities, exception model, validator, and Button/Switch/Dialog reference definitions, published as `docs/contracts/recipe-contract.md` and enforced by `gate:phase1` §9 on every pull request. RECIPE-002 is in progress: all 13 CSS recipe scopes have validated canonical definitions, and CI guards wrapper/slot coverage; CSS emission/migration remains. THEME-001 is complete: versioned semantic theme JSON schema (`tools/theme-contract-schema.ts`), validator with a WCAG-floor contrast check (`tools/theme-contract-validate.ts`, `tools/theme-contract-contrast.ts`), the first migration mechanism in the repo (`tools/theme-contract-migrate.ts`), and a `solidiom-default` reference theme cross-checked against BRAND-002's `tokens.css` for drift, published as `docs/contracts/theme-contract.md` and checked by `pnpm run theme:contract`. CLI-001 is complete: `packages/cli/src/source/` renamed to `src/source-install/` to stop colliding with the `src/`→`source/` emission root; `tools/audit-package-source-parity.ts` now generalizes RECIPE-006's parity audit to cover the CLI's tooling-tier source tree, `tools/emit-package-source.ts` adds an explicit regeneration/check script, and `gate:phase1` §9b asserts the collision is structurally resolved. Remaining M3 work: RECIPE-002b, RECIPE-003..006, CLI-002..010, THEME-002..005, BUILDER-001..006, BETA-001..003. |
