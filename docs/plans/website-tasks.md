...
`RECIPE-001` is complete; `docs/contracts/recipe-contract.md` is the normative reference and `docs/contracts/recipe-authoring-guide.md` is the authoring workflow. `RECIPE-002/003/004` are complete: the CSS, Tailwind, and UnoCSS emitters generate every shipped recipe (13 primitives × 3 profiles) from `tools/recipe-contract-definitions.ts`, with `pnpm run recipe:emit:{css,tailwind,unocss}:check` enforced in `gate:phase1`. `recipes-unocss` now ships a full recipe catalog matching the other two profiles, closing the gap it previously declared. Parity assertions and `src`/`source` build-parity checks remain in `RECIPE-005/006`.
