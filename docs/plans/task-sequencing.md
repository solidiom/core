...
| F6  | Only 3 of 15 recipe scopes are contract-backed. `tools/recipe-contract-definitions.ts` exports `buttonRecipe`, `switchRecipe`, `dialogRecipe`. `packages/recipes-{css,unocss}/src/recipes/` each hold 15 wrappers; `recipes-tailwind` holds 16 (`+typeset.tsx`). The other 12 wrappers are hand-authored and not generated from the contract.                                                                                                                     | `tools/recipe-contract-definitions.ts`; directory listings                                 |
...
| ----: | ------------------- | ---- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
|     1 | **COMP-001 Button** | M    | Variants + compound variants                            | Only item that is both contract-backed and already declares a component deliverable. Highest block fanout (30). |
...
Deliberately _not_ in the slice: `COMP-015 Switch`, despite being contract-backed. It
...
| Step | Item                     | Existing recipe wrapper? | Fanout | Blocks unlocked (cum.) |
| ---: | ------------------------ | ------------------------ | -----: | ---------------------: |
...
|   16 | COMP-015 Switch          | yes (contract-backed)    |     10 |                     24 |
...
- **Wrapper-exists group** (steps 5, 7, 8, 9, 11, 14, 15, 16, 18 — nine items): each needs
  a contract definition authored against `tools/recipe-contract-schema.ts` and the existing
  hand-authored wrapper reconciled against generated output. Expect drift; budget for it.
- **Greenfield group** (steps 4, 6, 10, 12, 13, 17, 19, 20, 21 — nine items): recipe
  definition, wrapper, and docs all new.
...
| R8  | Contract/wrapper drift in the nine wrapper-exists components | Silent divergence between generated and hand-authored output | `audit-recipe-drift.ts` and `recipe:emit:*:check` already exist; require both green in each component's DoD.            |
