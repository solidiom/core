...
**Status:** approved — D1–D6 resolved (§3); Phase 0 ready to start
...
| FOUND-002 | **L** | Registry v3: add `components[]`/`blocks[]`/`templates[]`/`themes[]` to the index, namespaced per-deliverable manifests, layer-aware discovery, and layer-aware `documentationMetadata()`. Preserve determinism, stable sort, and the integrity path. **Resized M→L; see the scope note below.**                                                                                                                                                                                                                                     | D2, D3                   |
...
| FOUND-005 | S     | `tools/block-catalog-gate.ts` + manifest validator: resolve every `componentDependencies` entry against `COMP-001..030` **by name** — comparing the `.json`, the `.md` companion, and §9.2, since disagreement between those three is what let ten misnumbered citations through (F12); `requiredStates` present, complete, and cardinality-matched against the prose `states` (D4); `proposedComponents` empty for every block, now a regression guard rather than a scheduling device (D6). Ratchet against the `Blocks` counter. | D4, D6, FOUND-002        |
| FOUND-006 | S     | `tools/scaffold-catalog-docs.ts` (mirror `scaffold-primitive-docs.ts`) emitting EN+ES stubs into `apps/site/src/content/{en,es}/{components,blocks,templates,themes}/` with real `translationSourceHash` values — never the 64-zero placeholder that caused `I18N-005`.                                                                                                                                                                                                                                                             | D3                       |
| FOUND-007 | M     | Route generators for `/components/[name]/`, `/blocks/[name]/`, `/templates/[name]/`, and `/themes/[name]/`, satisfying REG-007's one-deliverable-one-route invariant in both locales. Note `apps/site/src/pages/themes/` currently holds only `builder/`.                                                                                                                                                                                                                                                                           | FOUND-002                |
| FOUND-008 | S     | `recipe-emit-css.ts`: derive `solidiom-<scope>` by default, keep `CLASS_PREFIXES` for exceptions only (D5). Add the new gates to `ci.yml` and to `phase1-gate.ts` alongside the existing RECIPE-001..006 assertions — cite the section by title, not number, per `website-tasks.md` §3.1.                                                                                                                                                                                                                                           | D5, FOUND-004, FOUND-005 |
| FOUND-009 | S     | Close or except F6's wrapper asymmetry: `recipes-tailwind` carries `typeset.tsx` with no css/unocss counterpart, so "component = the wrapper set" is not uniformly true under D1(b). Either finish `RECIPE-008`(a) or record the exception in §8.2.                                                                                                                                                                                                                                                                                 | D1                       |
...
- `pnpm run recipe:emit:css:check` passes for a variant-bearing scope with no
  `CLASS_PREFIXES` entry, proving D5's derivation.
