...
Note: use `packages/*/source/**` (not `**/source/**`) to avoid excluding legitimate paths like `packages/cli/src/source-install/`.
...
**`@solidiom/cli` is a partial exception.** The CLI is a `layer:tooling` package with no `"solid"` export condition — its `source/` copy exists only for src/source parity convention consistency, not bundler dev-mode resolution, and it is git-tracked (not gitignored) like the rest of the CLI's `dist`-adjacent build state. Regenerate it with `pnpm run source:emit` after editing `packages/cli/src/**`, and verify with `pnpm run audit:package-source-parity`. The CLI's `src/source-install/` subdirectory (source-install engine and AST import rewriting) is unrelated: it is ordinary authored code, just named `source-install` instead of `source` specifically to avoid colliding with the `source/` emission root at the package's top level.
