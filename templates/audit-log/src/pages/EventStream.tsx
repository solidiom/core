...
const EVENTS: EventItem[] = [
  { id: "e1", timestamp: "2025-08-10 09:42:11", actor: "alice@example.com", action: "LOGIN", resource: "Web Console", severity: "success" },
  { id: "e2", timestamp: "2025-08-10 09:38:05", actor: "bob@example.com", action: "CREATE_USER", resource: "users/new", severity: "info" },
  { id: "e3", timestamp: "2025-08-10 09:30:22", actor: "system", action: "FAILED_LOGIN", resource: "API Gateway", severity: "warning" },
  { id: "e4", timestamp: "2025-08-10 09:15:47", actor: "carol@example.com", action: "UPDATE_ROLE", resource: "roles/admin", severity: "info" },
  { id: "e5", timestamp: "2025-08-10 08:55:33", actor: "eve@example.com", action: "DELETE_RECORD", resource: "logs/archive-2024", severity: "error" },
  { id: "e6", timestamp: "2025-08-10 08:42:19", actor: "frank@example.com", action: "EXPORT_DATA", resource: "reports/q2-2025", severity: "info" },
  { id: "e7", timestamp: "2025-08-10 08:30:01", actor: "system", action: "AUTO_SCALE", resource: "cluster/us-east-1", severity: "success" },
  { id: "e8", timestamp: "2025-08-10 08:15:44", actor: "henry@example.com", action: "MODIFY_PERMISSION", resource: "policies/data-access", severity: "warning" },
  { id: "e9", timestamp: "2025-08-10 07:58:12", actor: "iris@example.com", action: "CREATE_TEAM", resource: "teams/analytics", severity: "info" },
  { id: "e10", timestamp: "2025-08-10 07:45:08", actor: "jack@example.com", action: "ROTATE_KEY", resource: "secrets/api-keys", severity: "success" },
]
    ...
    <div class="space-y-8">
        ...
        <div>
          <Breadcrumb.Root class="mb-2">
            <Breadcrumb.List class="flex items-center gap-1.5 text-sm text-gray-500">
              ...
              <Breadcrumb.Item>
                <Breadcrumb.Link href="/" current class="text-gray-900 font-medium">Event Stream</Breadcrumb.Link>
              </Breadcrumb.Item>
            </Breadcrumb.List>
          </Breadcrumb.Root>
          <h1 class="text-2xl font-bold text-gray-900">Event Stream</h1>
          <p class="mt-1 text-sm text-gray-500">Real-time stream of audit events with actor, action, and resource details.</p>
        </div>
      ...
      <Alert.Root type="info" class="rounded-md border border-blue-200 bg-blue-50 p-4">
        ...
        <Alert.Description class="mt-1 text-sm text-blue-700">
          Showing {filtered().length} of {EVENTS.length} recent events. Stream auto-refreshes every 30 seconds.
        </Alert.Description>
      </Alert.Root>
        ...
        <Tabs.Content value="stats" class="pt-6">
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
            <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p class="mt-1 text-2xl font-bold text-blue-600">{EVENTS.filter((e) => e.severity === "info").length}</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
              <p class="text-sm font-medium text-gray-500">Warnings</p>
              <p class="mt-1 text-2xl font-bold text-yellow-600">{EVENTS.filter((e) => e.severity === "warning").length}</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
              <p class="text-sm font-medium text-gray-500">Errors</p>
              <p class="mt-1 text-2xl font-bold text-red-600">{EVENTS.filter((e) => e.severity === "error").length}</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
              <p class="text-sm font-medium text-gray-500">Success</p>
              <p class="mt-1 text-2xl font-bold text-green-600">{EVENTS.filter((e) => e.severity === "success").length}</p>
            </div>
          </div>
        </Tabs.Content>
      </Tabs.Root>
