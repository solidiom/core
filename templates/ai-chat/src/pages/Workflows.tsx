...
const WORKFLOWS: Workflow[] = [
  {
    ...
    description:
      "Extract key points from uploaded documents using GPT-4, then format as bullet points.",
    steps: 4,
    status: "active",
    lastRun: "5 min ago",
    avgDuration: "12s",
  },
  {
    ...
    description: "Process images through CLIP for classification, then DALL-E for style transfer.",
    steps: 6,
    status: "active",
    lastRun: "1 hour ago",
    avgDuration: "45s",
  },
  {
    ...
    description:
      "Generate code from natural language specs, run tests, and apply fixes iteratively.",
    steps: 5,
    status: "draft",
    lastRun: "Never",
    avgDuration: "—",
  },
  {
    ...
    description:
      "Analyze customer reviews for sentiment, extract entities, and generate summary reports.",
    steps: 3,
    status: "active",
    lastRun: "2 hours ago",
    avgDuration: "8s",
  },
  {
    ...
    description: "Clean, transform, and load data with LLM-assisted schema inference.",
    steps: 7,
    status: "archived",
    lastRun: "3 days ago",
    avgDuration: "2m 15s",
  },
  {
    ...
    description: "Run multiple LLM agents in a structured debate format to reach consensus.",
    steps: 8,
    status: "draft",
    lastRun: "Never",
    avgDuration: "—",
...
const statusLabel = (status: WorkflowStatus) => {
  switch (status) {
    ...
    case "draft":
      return { label: "Draft", class: "bg-gray-100 text-gray-600" }
    case "archived":
      return { label: "Archived", class: "bg-yellow-100 text-yellow-700" }
  }
}
    ...
    <div class="space-y-6">
        ...
        <div>
          <Breadcrumb.Root class="mb-2">
            <Breadcrumb.List class="flex items-center gap-1.5 text-sm text-gray-500">
              ...
              <Breadcrumb.Item>
                <Breadcrumb.Link href="/workflows" current class="text-gray-900 font-medium">
                  Workflows
                </Breadcrumb.Link>
              </Breadcrumb.Item>
            </Breadcrumb.List>
          ...
          <p class="mt-1 text-sm text-gray-500">
            Visual workflow builder for composing multi-step AI pipelines.
          </p>
        </div>
      ...
      <Alert.Root type="info" class="rounded-md border border-blue-200 bg-blue-50 p-4">
        ...
        <Alert.Description class="mt-1 text-sm text-blue-700">
          Compose workflows by chaining AI models, data transformations, and conditional logic.
          Active workflows can be triggered via API.
        </Alert.Description>
      </Alert.Root>
          ...
          <Card.Root class="rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
            <Card.Header class="flex items-start justify-between border-b border-gray-100 p-5 pb-3">
              <div class="flex-1">
                ...
                <span
                  class={`mt-1 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${statusLabel(wf.status).class}`}
                >
