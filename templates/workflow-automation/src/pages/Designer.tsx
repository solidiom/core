...
const WORKFLOWS = [
  { name: "Onboard New User", trigger: "user.signup", actions: ["send-email", "create-record", "notify-slack"], status: "active" as const },
  { name: "Process Refund", trigger: "order.refund_requested", actions: ["verify-purchase", "process-payment", "update-inventory"], status: "active" as const },
  { name: "Weekly Report", trigger: "cron.weekly", actions: ["aggregate-data", "generate-pdf", "send-email"], status: "paused" as const },
  { name: "Data Sync", trigger: "webhook.external", actions: ["transform", "validate", "upsert"], status: "draft" as const },
]
    ...
    <div class="space-y-8">
      <Breadcrumb.Root>
        <Breadcrumb.List class="flex items-center gap-2">
          ...
          <Breadcrumb.Item>
            <Breadcrumb.Link href="/" current class="text-sm font-medium text-gray-900">Designer</Breadcrumb.Link>
          </Breadcrumb.Item>
        </Breadcrumb.List>
      </Breadcrumb.Root>
      ...
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Visual drag-and-drop workflow designer with triggers and actions.</p>
      </div>
      ...
      <Alert.Root type="info" class="rounded-md border border-blue-200 bg-blue-50 p-4">
        ...
        <Alert.Description class="mt-1 text-sm text-blue-700">
          Trigger: <code class="rounded bg-blue-100 px-1 py-0.5 font-mono text-xs">user.signup</code> • 4 steps • Last run 2 minutes ago
        </Alert.Description>
      </Alert.Root>
      ...
      <div>
        ...
        <div class="space-y-4">
          {WORKFLOWS.map((wf) => (
            <WorkflowCard name={wf.name} trigger={wf.trigger} actions={wf.actions} status={wf.status} />
          ))}
        </div>
      </div>
