...
export function Playground(): JSX.Element {
  ...
  const [requestBody, setRequestBody] = createSignal('{"name": "John Doe", "email": "john@example.com"}')
  const [response, setResponse] = createSignal('{"status": "success", "data": [{"id": 1, "name": "John Doe", "email": "john@example.com"}, {"id": 2, "name": "Jane Smith", "email": "jane@example.com"}], "total": 2, "page": 1}')
  ...
  const runRequest = () => {
    ...
    setTimeout(() => {
      setIsRunning(false)
      setResponse('{"status": "success", "data": [{"id": 1, "name": "John Doe", "email": "john@example.com"}], "total": 1, "page": 1}')
    }, 800)
  }
      ...
      <div>
        <Breadcrumb.Root class="mb-2">
          <Breadcrumb.List class="flex items-center gap-1.5 text-sm text-gray-500">
            ...
            <Breadcrumb.Item>
              <Breadcrumb.Link href="/playground" current class="text-gray-900 font-medium">Playground</Breadcrumb.Link>
            </Breadcrumb.Item>
          </Breadcrumb.List>
        </Breadcrumb.Root>
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Interactive playground for testing API calls and SDK integrations.</p>
          </div>
              ...
              {SAMPLE_ENDPOINTS.map((ep) => (
                <button
                  class={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm hover:bg-gray-100 ${
                    ep.path === selectedPath() && ep.method === selectedMethod() ? "bg-indigo-50" : ""
                  }`}
                  onClick={() => { setSelectedMethod(ep.method); setSelectedPath(ep.path) }}
                >
