...
interface PipelineDef {
  ...
  name: string
  description: string
  steps: Array<{ id: string; name: string; status: "completed" | "running" | "pending" | "failed"; model?: string; duration?: string }>
  lastRun: string
...
const PIPELINES: PipelineDef[] = [
  {
    ...
    status: "completed",
    steps: [
      { id: "s1", name: "Upload & Parse", status: "completed", model: "document-parser", duration: "2.3s" },
      { id: "s2", name: "Entity Extraction", status: "completed", model: "gpt-4-turbo", duration: "4.1s" },
      { id: "s3", name: "Summarize", status: "completed", model: "claude-3-sonnet", duration: "3.8s" },
      { id: "s4", name: "Classify & Tag", status: "completed", model: "text-embedding-3-large", duration: "1.2s" },
    ],
  },
  {
    id: "2",
    ...
    steps: [
      ...
      { id: "s3", name: "LLM Review", status: "running", model: "gpt-4-turbo", duration: "12s / ..." },
  ...
  },
  {
    ...
    status: "failed",
    steps: [
      { id: "s1", name: "Fetch Tickets", status: "completed", model: "api-client", duration: "1.0s" },
      { id: "s2", name: "Sentiment Analysis", status: "completed", model: "claude-3-sonnet", duration: "6.4s" },
      { id: "s3", name: "Theme Extraction", status: "failed", model: "text-embedding-3-large", duration: "timeout" },
    ...
    <div class="space-y-8">
        ...
        <div>
          <Breadcrumb.Root class="mb-2">
            <Breadcrumb.List class="flex items-center gap-1.5 text-sm text-gray-500">
              ...
              <Breadcrumb.Item>
                <Breadcrumb.Link href="/" current class="text-gray-900 font-medium">Pipeline</Breadcrumb.Link>
              </Breadcrumb.Item>
            </Breadcrumb.List>
          </Breadcrumb.Root>
          <h1 class="text-2xl font-bold text-gray-900">Pipeline Builder</h1>
          <p class="mt-1 text-sm text-gray-500">Compose and visualize multi-step AI workflows with model assignments.</p>
        </div>
      ...
      <Alert.Root type="info" class="rounded-md border border-blue-200 bg-blue-50 p-4">
        ...
        <Alert.Description class="mt-1 text-sm text-blue-700">
          Each pipeline step can be assigned a model. Steps execute sequentially unless configured for parallel execution.
        </Alert.Description>
      </Alert.Root>
        ...
        {PIPELINES.map((pipeline) => (
          <Card.Root
            class={`cursor-pointer rounded-lg border bg-white shadow-sm transition-shadow hover:shadow-md ${
              selectedPipeline().id === pipeline.id ? "border-indigo-500 ring-1 ring-indigo-500" : "border-gray-200"
            }`}
          ...
          >
            <Card.Header class="border-b border-gray-100 p-5 pb-3">
              <div class="flex items-start justify-between">
                <div>
                  <Card.Title class="text-sm font-semibold text-gray-900">{pipeline.name}</Card.Title>
