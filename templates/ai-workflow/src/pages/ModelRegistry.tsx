...
const MODELS: RegistryEntry[] = [
  { id: "1", name: "gpt-4-turbo", type: "LLM", version: "2024-04-09", status: "registered", provider: "OpenAI", contextWindow: "128K", registeredAt: "Aug 1, 2024" },
  { id: "2", name: "claude-3-sonnet", type: "LLM", version: "1.0.0", status: "registered", provider: "Anthropic", contextWindow: "200K", registeredAt: "Jul 28, 2024" },
  { id: "3", name: "text-embedding-3-large", type: "Embedding", version: "1.2.0", status: "registered", provider: "OpenAI", contextWindow: "8K", registeredAt: "Jul 15, 2024" },
  { id: "4", name: "llama-3-8b-fine", type: "LLM", version: "0.9.3", status: "in_review", provider: "Meta (custom)", contextWindow: "8K", registeredAt: "Aug 5, 2024" },
  { id: "5", name: "dall-e-3", type: "Image Gen", version: "2.0.0", status: "registered", provider: "OpenAI", contextWindow: "—", registeredAt: "Jul 10, 2024" },
  { id: "6", name: "whisper-large-v3", type: "Speech", version: "1.0.0", status: "deprecated", provider: "OpenAI", contextWindow: "—", registeredAt: "Jun 1, 2024" },
  { id: "7", name: "clip-vit-large", type: "Vision", version: "1.5.0", status: "registered", provider: "OpenAI", contextWindow: "—", registeredAt: "Jul 20, 2024" },
]
...
const statusColor = (status: ModelStatus) => {
  switch (status) {
    ...
    case "deprecated": return "bg-gray-100 text-gray-600"
  }
}
    ...
    <div class="space-y-8">
        ...
        <div>
          <Breadcrumb.Root class="mb-2">
            <Breadcrumb.List class="flex items-center gap-1.5 text-sm text-gray-500">
              ...
              <Breadcrumb.Item>
                <Breadcrumb.Link href="/models" current class="text-gray-900 font-medium">Models</Breadcrumb.Link>
              </Breadcrumb.Item>
            </Breadcrumb.List>
          </Breadcrumb.Root>
          <h1 class="text-2xl font-bold text-gray-900">Model Registry</h1>
          <p class="mt-1 text-sm text-gray-500">Manage, version, and configure AI models for pipeline steps.</p>
        </div>
      ...
      <Alert.Root type="info" class="rounded-md border border-blue-200 bg-blue-50 p-4">
        ...
        <Alert.Description class="mt-1 text-sm text-blue-700">
          6 models registered, 1 under review. Register new models to make them available in pipeline steps.
        </Alert.Description>
      </Alert.Root>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Context</th>
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                {MODELS.map((model) => (
                  <tr class="hover:bg-gray-50">
                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{model.name}</td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{model.type}</td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{model.version}</td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{model.provider}</td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{model.contextWindow}</td>
                    <td class="whitespace-nowrap px-6 py-4">
                      <span class={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${statusColor(model.status)}`}>
                        <span class={`mr-1.5 h-1.5 w-1.5 rounded-full ${statusDot(model.status)}`} />
                        {model.status}
                      </span>
                    </td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{model.registeredAt}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Version</th>
                  <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Provider</th>
                  <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Context Window</th>
                  <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Status</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 bg-white">
