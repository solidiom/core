...
const POLICY_RULES: PolicyRule[] = [
  {
    ...
    description: "Detect and mask Social Security Numbers in all data stores",
  ...
  {
    id: "pol-003",
    name: "Email PII Classification",
    classification: "confidential",
    description: "Auto-classify fields containing email addresses as PII",
  ...
  {
    id: "pol-004",
    name: "Geolocation Access Control",
    classification: "confidential",
    description: "Restrict access to GPS coordinates to authorized roles",
    patterns: ["lat/lon pairs"],
    enforcement: "pending",
    owner: "Legal Team",
  ...
  },
  {
    ...
    classification: "restricted",
    description: "Encrypt PHI fields at rest and in transit",
    patterns: ["diagnosis codes", "patient IDs"],
    enforcement: "enforced",
    owner: "Compliance",
  ...
  {
    id: "pol-006",
    name: "Internal Docs Labeling",
    classification: "internal",
    description: "Auto-label documents marked for internal use only",
...
export function Classification(): JSX.Element {
  ...
  const filtered = () =>
    POLICY_RULES.filter(
      (rule) => classificationFilter() === "all" || rule.classification === classificationFilter(),
    )
    ...
    <div class="space-y-8">
      <div>
        <Breadcrumb.Root class="mb-2">
          <Breadcrumb.List class="flex items-center gap-1.5 text-sm text-gray-500">
            ...
            <Breadcrumb.Item>
              <Breadcrumb.Link href="/classification" current class="text-gray-900 font-medium">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
              Define sensitivity labels, apply classification rules, and enforce retention policies.
            </p>
          </div>
          <Button.Root
      ...
      <Alert.Root type="info" class="rounded-lg">
        ...
        <Alert.Description class="mt-1 text-sm text-gray-600">
          {POLICY_RULES.filter((r) => r.enforcement === "enforced").length} enforced,{" "}
          {POLICY_RULES.filter((r) => r.enforcement === "pending").length} pending,{" "}
          {POLICY_RULES.filter((r) => r.enforcement === "disabled").length} disabled out of{" "}
          {POLICY_RULES.length} total policies.
        </Alert.Description>
      </Alert.Root>
              ...
              {filtered().map((rule) => (
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-4 py-3">
                    <ClassificationBadge
                      level={
                        rule.classification as "public" | "internal" | "confidential" | "restricted"
                      }
                    />
                  </td>
                  <td class="px-4 py-3">
                    <span
                      class={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${enforcementColor(rule.enforcement)}`}
                    >
                      {rule.enforcement}
                    </span>
                  </td>
      ...
      <Dialog.Root open={showAddDialog} onOpenChange={setShowAddDialog}>
        <Dialog.Portal>
          ...
          <Dialog.Content class="fixed left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-xl">
            ...
            <p class="mt-1 text-sm text-gray-500">
              Define a new data classification and enforcement policy.
            </p>
            <div class="mt-4 space-y-3">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Phone Number Detection"
