    ...
    <div class="space-y-8">
        ...
        <div>
          <Breadcrumb.Root class="mb-2">
            <Breadcrumb.List class="flex items-center gap-1.5 text-sm text-gray-500">
              ...
              <Breadcrumb.Item>
                <Breadcrumb.Link href="/deployments" current class="text-gray-900 font-medium">
                  Deployments
                </Breadcrumb.Link>
              </Breadcrumb.Item>
            </Breadcrumb.List>
          ...
          <p class="mt-1 text-sm text-gray-500">
            Manage model deployment pipelines, rollbacks, and canary releases.
          </p>
        </div>
      ...
      <Alert.Root type="info" class="rounded-md border border-blue-200 bg-blue-50 p-4">
        ...
        <Alert.Description class="mt-1 text-sm text-blue-700">
          claude-3-sonnet v1.0.1 is canary-deploying to staging (20% traffic). Monitor for 30
          minutes before promoting.
        </Alert.Description>
      </Alert.Root>
                ...
                {DEPLOYMENTS.filter(
                  ...
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="whitespace-nowrap px-6 py-4">
                      <span
                        class={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${statusColor(dep.status)}`}
                      >
        ...
        <Tabs.Content value="history" class="pt-6">
          <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                {DEPLOYMENTS.filter((d) => d.status === "completed" || d.status === "failed").map(
                  (dep) => (
                    <tr class="hover:bg-gray-50">
                      <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">
                        {dep.model}
                      ...
                      <td class="whitespace-nowrap px-6 py-4">
                        ...
                        >
                          <span
                            class={`mr-1.5 h-1.5 w-1.5 rounded-full ${statusDot(dep.status)}`}
                          />
                          {dep.status}
                        </span>
                      </td>
